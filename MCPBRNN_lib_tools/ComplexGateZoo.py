...
class MCPBRNN_Generic_OutputANNGate_PETconstraint(nn.Module):
    ...
    def __init__(self,
        ...
        super(MCPBRNN_Generic_OutputANNGate_PETconstraint, self).__init__()
    ...
    def forward(self, x, epoch, time_lag, y_obs,cmean,cstd):
        ...
        for b in range(0+time_lag, batch_size):            
            ...
            # calculate gates            
                ...
                u2_Gate = (u2.expand(1, self.gate_dim_l))  
                c_0_Gate = (c_0.expand(1, self.gate_dim_o))  

                ib = 0 

                oo1 = torch.exp(self.weight_r_yom)/(torch.exp(self.weight_r_yom) + torch.exp(self.weight_r_ylm) + torch.exp(self.weight_r_yfm))                             
                oo2 = self.relu_o((c_0_Gate-mo)/so - self.relu_bias_o)
                oo = oo1 * torch.sigmoid((torch.addmm(self.bias_ln_yom,oo2,self.weight_b1_yom)))

                ol1 = torch.exp(self.weight_r_ylm)/(torch.exp(self.weight_r_yom) + torch.exp(self.weight_r_ylm) + torch.exp(self.weight_r_yfm))
                ol2 = torch.addmm(bias_b0_ylm, (u2-ml)/sl, self.weight_b2_ylm)
        ...
        return h_n, c_n, l_n, lc_n, bp_n, Gate_ib, Gate_oo, Gate_ol, Gate_ol_constraint, Gate_f, h_nout, obs_std
...
class MCPBRNN_Generic_LossANNGate_PETconstraint(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_size: int = 1,
                 initial_forget_bias: int = 0):
        super(MCPBRNN_Generic_LossANNGate_PETconstraint, self).__init__()
    ...
    def forward(self, x, epoch, time_lag, y_obs,cmean,cstd):
        ...
        for b in range(0+time_lag, batch_size):            
            ...
            # calculate gates            
                ...
                oo1 = torch.exp(self.weight_r_yom)/(torch.exp(self.weight_r_yom) + torch.exp(self.weight_r_ylm) + torch.exp(self.weight_r_yfm))                             
                oo2 = torch.addmm(bias_b0_yom, (c_0_Gate-mo)/so, self.weight_b1_yom)
                oo = oo1 * torch.sigmoid(oo2)

                ol1 = torch.exp(self.weight_r_ylm)/(torch.exp(self.weight_r_yom) + torch.exp(self.weight_r_ylm) + torch.exp(self.weight_r_yfm))
                ol2 = self.relu_l((u2_Gate-ml)/sl - self.relu_bias_l)               
                ol = ol1 * torch.sigmoid(torch.addmm(self.bias_ln_ylm, ol2, self.weight_b2_ylm))
...
class MCPBRNN_Generic_ANNGate_PETconstraint_Generic(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_size: int = 1,
                 initial_forget_bias: int = 0):
        super(MCPBRNN_Generic_ANNGate_PETconstraint_Generic, self).__init__()
