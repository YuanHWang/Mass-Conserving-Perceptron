...
class MCPBRNN_Generic_PETconstraint_MIoutput(nn.Module):
    ...
    def __init__(self,
        ...
        super(MCPBRNN_Generic_PETconstraint_MIoutput, self).__init__()
...
class MCPBRNN_Generic_PETconstraint_MIoutput_Sigmoid(nn.Module):
    ...
    def __init__(self,
                 ...
                 hidden_size: int = 1,
                 initial_forget_bias: int = 0):
        super(MCPBRNN_Generic_PETconstraint_MIoutput_Sigmoid, self).__init__()
        ...
        self.weight_b1_yom = nn.Parameter(torch.FloatTensor(self.gate_dim_o, self.gate_dim_o))        
        self.weight_b2_yom = nn.Parameter(torch.FloatTensor(self.gate_dim_o, self.hidden_size))
        self.bias_b0_ylm = nn.Parameter(torch.FloatTensor(self.hidden_size))
        self.weight_b2_ylm = nn.Parameter(torch.FloatTensor(self.gate_dim_l, self.hidden_size))
    ...
    def reset_parameters(self):
        ...
        self.weight_b1_yom = nn.Parameter(torch.rand(self.gate_dim_o, self.gate_dim_o))        
        self.weight_b2_yom = nn.Parameter(torch.rand(self.gate_dim_o, self.hidden_size))
        self.bias_b0_ylm = nn.Parameter(torch.rand(self.hidden_size))
        self.weight_b2_ylm = nn.Parameter(torch.rand(self.gate_dim_l, self.hidden_size))
