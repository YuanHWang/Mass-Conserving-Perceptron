...
Ini_dir = "results_MR/MR-dependent-New-Iter2/MCPBRNN_PETconstraint_MCRelaxed_Generic_Iter1_1Layer_1node_1/model_epoch19.pt"
...
Out_ov_flux = np.zeros((timeLen,1))
...
class Model(nn.Module):
    ...
    def forward(self, x_d, epoch, time_lag, y_eval,c_mean,c_std):
        ...
        h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, ol_c, f, h_nout, obs_std, ov, ovflux = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval,c_mean,c_std)    
        last_h = self.dropout(h_t)      
        out = last_h        
        return out, h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, ol_c, f, h_nout, obs_std, ov, ovflux#, std_flag
...
for epoch in range(1,1+1): 
    ...
    mr_flux = mr_flux.detach().cpu().numpy()    
...
# Output time series
    ...
    Out_f_gate = f_gate [:,0]
    Out_ov_gate = mr_gate[:,0]
    Out_ov_flux = mr_flux[:,0]
    print(B2)

# Save Results file

CaseName = "results_MR/MR-dependent-New-Iter2/"
...
Out_file = pd.DataFrame(Out_ov_flux)
outname = CaseName + '/' + 'Out_fluxmr_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)