...
weight_IC_file = "results_MIX/MR-dependent-MI-outloss-sigmoid-Iter1/MCPBRNN_PETconstraint_Generic_1Layer_1node_MI_outdim1_MI_lossdim1_1_MR_Regular/model_epoch24.pt"
...
for epoch in range(1,1+1): 
    ...
    mr_gate = mr_gate.detach().cpu().numpy()
    mr_eval = mr_eval.detach().cpu().numpy()      
...
# Output time series
    ...
    Out_mr_gate [0:timeLen,epoch-1] = mr_gate[:,0]
    Out_mr [0:timeLen,epoch-1] = mr_eval[:,0]
...
outname = CaseName + '/' + 'Out_gatemr_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_mr)
outname = CaseName + '/' + 'Out_mr_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)