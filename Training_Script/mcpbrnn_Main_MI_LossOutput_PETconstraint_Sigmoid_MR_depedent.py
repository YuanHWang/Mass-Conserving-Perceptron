...
from MCPBRNN_lib_tools.NodeZoo_multilossGate import MCPBRNN_Generic_PETconstraint_MIloss_Sigmoid
from MCPBRNN_lib_tools.NodeZoo_multioutputGate import MCPBRNN_Generic_PETconstraint_MIoutput_Sigmoid
from MCPBRNN_lib_tools.NodeZoo_multi_lossoutput_Gate import MCPBRNN_Generic_PETconstraint_MIoutputloss_Sigmoid
from MCPBRNN_lib_tools.MRZoo import MCPBRNN_PETconstraint_MassRelax_Regular, MCPBRNN_PETconstraint_MassRelax_Regular_MI_outputloss_sigmoid
...
CaseName = 'MCPBRNN_PETconstraint_Generic_' + str(depth_size) + 'Layer_1node_' + 'MI_outdim' + str(gate_dim_o) + '_MI_lossdim' + str(gate_dim_l) + '_' + str(cfg["case_no"]) + "_MR_Regular"
directory = CaseName
...
class Model_old(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode = MCPBRNN_Generic_PETconstraint_MIoutputloss_Sigmoid(input_size=input_size_dyn,
...
class Model_old_MR(nn.Module):
    ...
    def __init__(self,
        ...
        no_static: bool
            If True, runs standard LSTM
        """
        super(Model_old_MR, self).__init__()
        self.input_size_dyn = input_size_dyn
        self.input_size_stat = input_size_stat
        self.hidden_size = hidden_size
        self.spinLen = spinLen
        ...
        self.dropout_rate = dropout       
        self.batch_size = batch_size        
        self.MCPBRNNNode = MCPBRNN_PETconstraint_MassRelax_Regular(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
    ...
    def forward(self, x_d, epoch, time_lag, y_eval,c_mean,c_std):
        ...
        h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, ol_c, f, h_nout, obs_std, ov, ov_t = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval,c_mean,c_std)    
        last_h = self.dropout(h_t)      
        out = last_h        
        return out, h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, ol_c, f, h_nout, obs_std, ov, ov_t#, std_flag

model_old_MR = Model_old_MR(input_size_dyn=input_size_dyn,
              input_size_stat=input_size_stat,
              hidden_size=hidden_size,
              spinLen=spinLen,
              traintimeLen=traintimeLen,              
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.dropout_rate = dropout       
        self.batch_size = batch_size        
        self.MCPBRNNNode = MCPBRNN_PETconstraint_MassRelax_Regular_MI_outputloss_sigmoid(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         gate_dim_ucorr=gate_dim_ucorr,  
                         spinLen=spinLen,
...
weight_IC_file = "MI-outputlossGate-dim1-sigmoid/MCPBRNN_PETconstraint_Generic_1Layer_1node_MI_outdim1_MI_lossdim1_1/model_epoch18.pt"
model_old.load_state_dict(torch.load(weight_IC_file))   

weight_IC_file = "MR-dependent-New-Iter2/MCPBRNN_PETconstraint_MCRelaxed_Generic_Iter1_1Layer_1node_1/model_epoch19.pt"
model_old_MR.load_state_dict(torch.load(weight_IC_file))  
...
model.state_dict()["MCPBRNNNode.weight_b2_yom"][:] = model_old.state_dict()["MCPBRNNNode.weight_b2_yom"][:]
model.state_dict()["MCPBRNNNode.bias_b0_ylm"][:] = model_old.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]
model.state_dict()["MCPBRNNNode.weight_b1_ylm"][:] = model_old.state_dict()["MCPBRNNNode.weight_b1_ylm"][:]
model.state_dict()["MCPBRNNNode.weight_b2_ylm"][:] = model_old.state_dict()["MCPBRNNNode.weight_b2_ylm"][:]
model.state_dict()["MCPBRNNNode.weight_r_yvm"][:] = model_old_MR.state_dict()["MCPBRNNNode.weight_r_yvm"][:]
model.state_dict()["MCPBRNNNode.weight_s_yvm"][:] = model_old_MR.state_dict()["MCPBRNNNode.weight_s_yvm"][:]
model.state_dict()["MCPBRNNNode.bias_b0_yrm"][:] = model_old_MR.state_dict()["MCPBRNNNode.bias_b0_yrm"][:]
...
# Output time series
    ...
    OutMatrix[epoch-1,6]= model.state_dict()["MCPBRNNNode.weight_b2_yom"][:]    
    OutMatrix[epoch-1,7]= model.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]
    OutMatrix[epoch-1,8]= model.state_dict()["MCPBRNNNode.weight_b1_ylm"][:]    
    OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode.weight_b2_ylm"][:]
    OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode.weight_s_yvm"][:]    
    OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode.bias_b0_yrm"][:]
...
# Save Results file
outtofile = pd.DataFrame(OutMatrix, columns=["MCPBRNNNode.weight_r_yom","MCPBRNNNode.weight_r_ylm","MCPBRNNNode.weight_r_yfm","MCPBRNNNode.weight_r_yvm","MCPBRNNNode.bias_b0_yom","MCPBRNNNode.weight_b1_yom","MCPBRNNNode.weight_b2_yom","MCPBRNNNode.bias_b0_ylm","MCPBRNNNode.weight_b1_ylm","MCPBRNNNode.weight_b2_ylm","MCPBRNNNode.weight_s_yvm","MCPBRNNNode.bias_b0_yrm",'NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])
