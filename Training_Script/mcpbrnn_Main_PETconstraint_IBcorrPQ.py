...
import csv
# import class
from MCPBRNN_lib_tools.InputBiasCorr import MCPBRNN_PETconstraint_IBcorrPQ_Generic
...
CaseName = 'MCPBRNN_PETconstraint_Generic_IBCorrPQ_dim' + str(gate_dim_ucorr) + '_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"]) 
directory = CaseName
...
if gate_dim_ucorr == 1:
    par_no = 10
elif gate_dim_ucorr == 2:
    par_no = 12 
elif gate_dim_ucorr == 3:
    par_no = 14 
elif gate_dim_ucorr == 4:
    par_no = 16 
elif gate_dim_ucorr == 5:
    par_no = 18 
...
class Model_pre(nn.Module):
    ...
    def __init__(self,
        ...
        self.dropout_rate = dropout       
        self.batch_size = batch_size        
        self.MCPBRNNNode = MCPBRNN_PETconstraint_IBcorrPQ_Generic(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         gate_dim_ucorr=gate_dim_ucorr - 1,  
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
                         initial_forget_bias=initial_forget_bias)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, x_d, epoch, time_lag, y_eval,c_mean,c_std):
        ...
        h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, olc, f, u1BC_t, h_nout, obs_std = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval,c_mean,c_std)    
        last_h = self.dropout(h_t)      
        out = last_h        
        return out, h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, olc, f, u1BC_t
...
model_pre = Model_pre(input_size_dyn=input_size_dyn,
              input_size_stat=input_size_stat,
              hidden_size=hidden_size,
              spinLen=spinLen,
              traintimeLen=traintimeLen,              
...
if gate_dim_ucorr == 1:
    ...
    YHW= 1.0 + np.random.normal(0,0.1,size=(gate_dim_ucorr,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_b"][0][0]= torch.Tensor(YHW)
    YHW=np.random.normal(0,1,size=(gate_dim_ucorr,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_wj"][0][0]= torch.Tensor(np.absolute(YHW))
    YHW=np.random.normal(0,0.25,size=(1, gate_dim_ucorr))
    model.state_dict()["MCPBRNNNode.relu_bj"][0][0]= torch.Tensor(np.absolute(YHW))

elif gate_dim_ucorr == 2:

    weight_file = "IBCorrPQ-dim1/MCPBRNN_PETconstraint_Generic_IBCorrPQ_dim1_1Layer_1node_6/model_epoch17.pt"  
    model_pre.load_state_dict(torch.load(weight_file))

    model.state_dict()["MCPBRNNNode.weight_r_yom"][:]= model_pre.state_dict()["MCPBRNNNode.weight_r_yom"][:]
    model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]= model_pre.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
    model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]= model_pre.state_dict()["MCPBRNNNode.weight_r_yfm"][:]

    model.state_dict()["MCPBRNNNode.bias_b0_yom"][:]= model_pre.state_dict()["MCPBRNNNode.bias_b0_yom"][:]
    model.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]= model_pre.state_dict()["MCPBRNNNode.bias_b0_ylm"][:]
    model.state_dict()["MCPBRNNNode.weight_b2_ylm"][:] = model_pre.state_dict()["MCPBRNNNode.weight_b2_ylm"][:]
    model.state_dict()["MCPBRNNNode.weight_b1_yom"][:] = model_pre.state_dict()["MCPBRNNNode.weight_b1_yom"][:]

    model.state_dict()["MCPBRNNNode.ln_b"][:]= model_pre.state_dict()["MCPBRNNNode.ln_b"][:]    
    model.state_dict()["MCPBRNNNode.ln_wj"][0][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][0][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][0]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][0]
    YHW=np.random.normal(0,1,size=(1,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_wj"][1][0]= torch.Tensor(np.absolute(YHW))
    YHW=np.random.normal(0,0.25,size=(1, 1))
    model.state_dict()["MCPBRNNNode.relu_bj"][0][1]= torch.Tensor(np.absolute(YHW))

elif gate_dim_ucorr == 3:

    weight_file = "IBCorrPQ-dim2/MCPBRNN_PETconstraint_Generic_IBCorrPQ_dim2_1Layer_1node_7/model_epoch20.pt"
    model_pre.load_state_dict(torch.load(weight_file))
    ...
    model.state_dict()["MCPBRNNNode.ln_b"][:]= model_pre.state_dict()["MCPBRNNNode.ln_b"][:]      
    model.state_dict()["MCPBRNNNode.ln_wj"][0][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][0][0]
    model.state_dict()["MCPBRNNNode.ln_wj"][1][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][1][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][0]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][1]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][1]
    YHW=np.random.normal(0,1,size=(1,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_wj"][2][0]= torch.Tensor(np.absolute(YHW))
    YHW=np.random.normal(0,0.25,size=(1, 1))
    model.state_dict()["MCPBRNNNode.relu_bj"][0][2]= torch.Tensor(np.absolute(YHW))

elif gate_dim_ucorr == 4:

    weight_file = "IBCorrPQ-dim3/MCPBRNN_PETconstraint_Generic_IBCorrPQ_dim3_1Layer_1node_3/model_epoch6.pt"
    model_pre.load_state_dict(torch.load(weight_file))
    ...
    model.state_dict()["MCPBRNNNode.ln_wj"][0][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][0][0]
    model.state_dict()["MCPBRNNNode.ln_wj"][1][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][1][0]
    model.state_dict()["MCPBRNNNode.ln_wj"][2][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][2][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][0]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][1]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][1]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][2]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][2]
    YHW=np.random.normal(0,1,size=(1,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_wj"][3][0]= torch.Tensor(np.absolute(YHW))
    YHW=np.random.normal(0,0.25,size=(1, 1))
    model.state_dict()["MCPBRNNNode.relu_bj"][0][3]= torch.Tensor(np.absolute(YHW))

elif gate_dim_ucorr == 5:

    weight_file = "IBCorrPQ-dim4/MCPBRNN_PETconstraint_Generic_IBCorrPQ_dim4_1Layer_1node_2/model_epoch17.pt"
    model_pre.load_state_dict(torch.load(weight_file))
    ...
    model.state_dict()["MCPBRNNNode.ln_wj"][1][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][1][0]
    model.state_dict()["MCPBRNNNode.ln_wj"][2][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][2][0]
    model.state_dict()["MCPBRNNNode.ln_wj"][3][0]= model_pre.state_dict()["MCPBRNNNode.ln_wj"][3][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][0]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][0]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][1]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][1]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][2]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][2]
    model.state_dict()["MCPBRNNNode.relu_bj"][0][3]= model_pre.state_dict()["MCPBRNNNode.relu_bj"][0][3]
    YHW=np.random.normal(0,1,size=(1,hidden_size))
    model.state_dict()["MCPBRNNNode.ln_wj"][4][0]= torch.Tensor(np.absolute(YHW))
    YHW=np.random.normal(0,0.25,size=(1, 1))
    model.state_dict()["MCPBRNNNode.relu_bj"][0][4]= torch.Tensor(np.absolute(YHW))
...
for epoch in range(1,num_epochs+1): 
    ...
    u1_BC_eval = u1_BC_eval.detach().cpu().numpy() 
    ...
    if gate_dim_ucorr == 1:
        ...
        OutMatrix[epoch-1,7]= model.state_dict()["MCPBRNNNode.ln_b"][:]
        OutMatrix[epoch-1,8]= model.state_dict()["MCPBRNNNode.ln_wj"][0][0]
        OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode.relu_bj"][0][0]  
    elif gate_dim_ucorr == 2:
        OutMatrix[epoch-1,0]= model.state_dict()["MCPBRNNNode.weight_r_yom"][:]
        OutMatrix[epoch-1,1]= model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
        OutMatrix[epoch-1,2]= model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
        ...
        OutMatrix[epoch-1,8]= model.state_dict()["MCPBRNNNode.ln_wj"][0][0]
        OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode.relu_bj"][0][0]
        OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode.ln_wj"][1][0]
        OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode.relu_bj"][0][1] 
    elif gate_dim_ucorr == 3:
        OutMatrix[epoch-1,0]= model.state_dict()["MCPBRNNNode.weight_r_yom"][:]
        OutMatrix[epoch-1,1]= model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
        OutMatrix[epoch-1,2]= model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
        ...
        OutMatrix[epoch-1,10]= model.state_dict()["MCPBRNNNode.ln_wj"][1][0]
        OutMatrix[epoch-1,11]= model.state_dict()["MCPBRNNNode.relu_bj"][0][1] 
        OutMatrix[epoch-1,12]= model.state_dict()["MCPBRNNNode.ln_wj"][2][0]
        OutMatrix[epoch-1,13]= model.state_dict()["MCPBRNNNode.relu_bj"][0][2]        
    elif gate_dim_ucorr == 4:
        OutMatrix[epoch-1,0]= model.state_dict()["MCPBRNNNode.weight_r_yom"][:]
        OutMatrix[epoch-1,1]= model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
        OutMatrix[epoch-1,2]= model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
        ...
        OutMatrix[epoch-1,12]= model.state_dict()["MCPBRNNNode.ln_wj"][2][0]
        OutMatrix[epoch-1,13]= model.state_dict()["MCPBRNNNode.relu_bj"][0][2]  
        OutMatrix[epoch-1,14]= model.state_dict()["MCPBRNNNode.ln_wj"][3][0]
        OutMatrix[epoch-1,15]= model.state_dict()["MCPBRNNNode.relu_bj"][0][3]       
    elif gate_dim_ucorr == 5:
        OutMatrix[epoch-1,0]= model.state_dict()["MCPBRNNNode.weight_r_yom"][:]
        OutMatrix[epoch-1,1]= model.state_dict()["MCPBRNNNode.weight_r_ylm"][:]
        OutMatrix[epoch-1,2]= model.state_dict()["MCPBRNNNode.weight_r_yfm"][:]
        ...
        OutMatrix[epoch-1,14]= model.state_dict()["MCPBRNNNode.ln_wj"][3][0]
        OutMatrix[epoch-1,15]= model.state_dict()["MCPBRNNNode.relu_bj"][0][3] 
        OutMatrix[epoch-1,16]= model.state_dict()["MCPBRNNNode.ln_wj"][4][0]
        OutMatrix[epoch-1,17]= model.state_dict()["MCPBRNNNode.relu_bj"][0][4]            
# Output time series
...
if gate_dim_ucorr == 1:
    outtofile = pd.DataFrame(OutMatrix, columns=['MCPBRNNNode.weight_r_yom','MCPBRNNNode.weight_r_ylm','MCPBRNNNode.weight_r_yfm','MCPBRNNNode.bias_b0_yom','MCPBRNNNode.weight_b1_yom','MCPBRNNNode.bias_b0_ylm','MCPBRNNNode.weight_b2_ylm','MCPBRNNNode.ln_b','MCPBRNNNode.ln_wj','MCPBRNNNode.relu_bj','NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])

elif gate_dim_ucorr == 2:
    outtofile = pd.DataFrame(OutMatrix, columns=['MCPBRNNNode.weight_r_yom','MCPBRNNNode.weight_r_ylm','MCPBRNNNode.weight_r_yfm','MCPBRNNNode.bias_b0_yom','MCPBRNNNode.weight_b1_yom','MCPBRNNNode.bias_b0_ylm','MCPBRNNNode.weight_b2_ylm','MCPBRNNNode.ln_b','MCPBRNNNode.ln_wj','MCPBRNNNode.relu_bj','MCPBRNNNode.ln_wj2','MCPBRNNNode.relu_bj2','NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])

elif gate_dim_ucorr == 3:
    outtofile = pd.DataFrame(OutMatrix, columns=['MCPBRNNNode.weight_r_yom','MCPBRNNNode.weight_r_ylm','MCPBRNNNode.weight_r_yfm','MCPBRNNNode.bias_b0_yom','MCPBRNNNode.weight_b1_yom','MCPBRNNNode.bias_b0_ylm','MCPBRNNNode.weight_b2_ylm','MCPBRNNNode.ln_b','MCPBRNNNode.ln_wj','MCPBRNNNode.relu_bj','MCPBRNNNode.ln_wj2','MCPBRNNNode.relu_bj2','MCPBRNNNode.ln_wj3','MCPBRNNNode.relu_bj3','NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])

elif gate_dim_ucorr == 4:
    outtofile = pd.DataFrame(OutMatrix, columns=['MCPBRNNNode.weight_r_yom','MCPBRNNNode.weight_r_ylm','MCPBRNNNode.weight_r_yfm','MCPBRNNNode.bias_b0_yom','MCPBRNNNode.weight_b1_yom','MCPBRNNNode.bias_b0_ylm','MCPBRNNNode.weight_b2_ylm','MCPBRNNNode.ln_b','MCPBRNNNode.ln_wj','MCPBRNNNode.relu_bj','MCPBRNNNode.ln_wj2','MCPBRNNNode.relu_bj2','MCPBRNNNode.ln_wj3','MCPBRNNNode.relu_bj3','MCPBRNNNode.ln_wj4','MCPBRNNNode.relu_bj4','NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])

elif gate_dim_ucorr == 5:
    outtofile = pd.DataFrame(OutMatrix, columns=['MCPBRNNNode.weight_r_yom','MCPBRNNNode.weight_r_ylm','MCPBRNNNode.weight_r_yfm','MCPBRNNNode.bias_b0_yom','MCPBRNNNode.weight_b1_yom','MCPBRNNNode.bias_b0_ylm','MCPBRNNNode.weight_b2_ylm','MCPBRNNNode.ln_b','MCPBRNNNode.ln_wj','MCPBRNNNode.relu_bj','MCPBRNNNode.ln_wj2','MCPBRNNNode.relu_bj2','MCPBRNNNode.ln_wj3','MCPBRNNNode.relu_bj3','MCPBRNNNode.ln_wj4','MCPBRNNNode.relu_bj4','MCPBRNNNode.ln_wj5','MCPBRNNNode.relu_bj5','NSE','KGE','KGE-A','KGE-B','Corr','mse','KGEss','NSE_selection','KGE_selection','KGE-A_selection','KGE-B_selection','Corr_selection','mse_selection','KGEss_selection','NSE_testing','KGE_testing','KGE-A_testing','KGE-B_testing','Corr_testing','mse_testing','KGEss_testing','NSE_spinup','KGE_spinup','KGE-A_spinup','KGE-B_spinup','Corr_spinup','mse_spinup','KGEss_spinup','KGEtimelag_1','KGEtimelag_2','KGEtimelag_3'])
