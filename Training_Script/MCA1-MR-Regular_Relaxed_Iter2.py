...
from MCPBRNN_lib_tools.MRZoo_Relaxed import MCPBRNN_PETconstraint_MassRelax_Regular
...
CaseName = 'MCPBRNN_PETconstraint_MCRelaxed_Generic_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"]) + '_Relaxed'
directory = CaseName
...
#Ini_dir = "results_temp/M5-case_Trial1/MCPBRNN_Generic_PETconstraint_Scaling_M5_1Layer_1node_1/model_epoch10.pt"
Ini_dir = "MR-dependent-Relax-Iter2/MCPBRNN_PETconstraint_MCRelaxed_Generic_1Layer_1node_1_Relaxed/model_epoch3.pt"
...
class Model(nn.Module):
    ...
    def forward(self, x_d, epoch, time_lag, y_eval,c_mean,c_std):
        ...
        out = last_h        
        return out, h_t, c_t, l_t, lc_t, bp_t, i, oo, ol, ol_c, f, h_nout, obs_std, ov#, std_flag
...
YHW=np.array(np.random.normal(0, 0.05, 1))
model.state_dict()["MCPBRNNNode.weight_r_yvm"][:]= torch.Tensor(YHW)
YHW=np.array(np.random.normal(0, 0.05, 1))
model.state_dict()["MCPBRNNNode.weight_s_yvm"][:]= torch.Tensor(YHW)
YHW=np.array(np.random.normal(0, 0.05, 1))
model.state_dict()["MCPBRNNNode.bias_b0_yrm"][:]= torch.Tensor(YHW)
'''
for name, param in model.state_dict().items():
    print(name)
    print(param)
...
for epoch in range(1,num_epochs+1): 
    ...
    OutMatrix[epoch-1,9]= model.state_dict()["MCPBRNNNode.bias_b0_yrm"][:]
