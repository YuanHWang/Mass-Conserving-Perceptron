...
import glob 
# import class
from MCPBRNN_lib_tools.BMZoo import Vanilla_RNN_Sigmoid_Qsim_SingleNode
...
learning_rate = 0.05
learning_rates = {500: 0.025, 1000: 0.0125}

# Define case & directory
CaseName = 'Vanilla_RNN_Sigmoid_Qsim-caseno' + str(cfg["case_no"]) + "_" + "PP_Lag_"  +  str(PP_input) + "Q_Lag_" + str(Q_input) + "PET_Lag_" + str(PET_input) + '_Sigmoid_' + str(hidden_size) + '_Node'
directory = CaseName
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode = Vanilla_RNN_Sigmoid_Qsim_SingleNode(input_size=input_size_dyn,
                                                 hidden_size=hidden_size)
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x_d):

        h_t,x_h,x_hs = self.MCPBRNNNode(x_d)    
