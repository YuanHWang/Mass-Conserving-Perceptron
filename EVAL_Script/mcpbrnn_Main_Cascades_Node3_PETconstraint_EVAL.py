...
import csv
# import class
from MCPBRNN_lib_tools.CascadeZoo_New import MCPBRNN_Cascade_2tank_PETconstraint_Generic_New, MCPBRNN_Cascade_3tank_PETconstraint_Generic_New
from MCPBRNN_lib_tools.Eval_Metric import ANLL_out, correlation, NS, KGE
from MCPBRNN_lib_tools.Loss_Function import  KGELoss, ANLL_type1, ANLL_type2
...
parser.add_argument('--mo2',
                        type=float,
                        default=412.9139085,
                        help="")

parser.add_argument('--so2',
                        type=float,
                        default=77.49943867,
                        help="")

parser.add_argument('--mo3',
                        type=float,
                        default=412.9139085,
                        help="")

parser.add_argument('--so3',
                        type=float,
                        default=77.49943867,
                        help="")
...
CaseName = 'MCPBRNN_PETconstraint_Generic_Cascade3_' + str(depth_size) + 'Layer_1node_' + str(cfg["case_no"])
directory = CaseName
...
Ini_dir = "results_Cascade/Cascade-3Nodes-New/MCPBRNN_PETconstraint_Generic_Cascade3_1Layer_1node_6/model_epoch37.pt"
...
par_no = 27 # Temporarily not save the parameter value
skillmetric = 7
...
Out_hidden_C2 = np.zeros((timeLen,1))
Out_cell_C2 = np.zeros((timeLen,1))
Out_loss_C2 = np.zeros((timeLen,1))
Out_lossc_C2 = np.zeros((timeLen,1))
Out_o_gate_C2 = np.zeros((timeLen,1))
Out_l_gate_C2 = np.zeros((timeLen,1))
Out_lc_gate_C2 = np.zeros((timeLen,1))
Out_f_gate_C2 = np.zeros((timeLen,1))

Out_hidden_C3 = np.zeros((timeLen,1))
Out_cell_C3 = np.zeros((timeLen,1))
Out_loss_C3 = np.zeros((timeLen,1))
Out_lossc_C3 = np.zeros((timeLen,1))
Out_o_gate_C3 = np.zeros((timeLen,1))
Out_l_gate_C3 = np.zeros((timeLen,1))
Out_lc_gate_C3 = np.zeros((timeLen,1))
Out_f_gate_C3 = np.zeros((timeLen,1))
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode = MCPBRNN_Cascade_3tank_PETconstraint_Generic_New(input_size=input_size_dyn,
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, mo, so, mo2, so2, mo3, so3, mu, su, mu2, su2):
        ...
        out_hn, h_t, c_t, l_t, lc_t, bp_t, ib, oo, ol, f, olc, h_t_tank2, c_t_tank2, l_t_tank2, lc_t_tank2, bp_t_tank2, ib_tank2, Gate_oo_tank2, ol_tank2, f_tank2, olc_tank2, h_t_tank3, c_t_tank3, l_t_tank3, lc_t_tank3, bp_t_tank3, ib_tank3, Gate_oo_tank3, ol_tank3, f_tank3, olc_tank3 = self.MCPBRNNNode(x_d, epoch, time_lag, y_eval, mo, so, mo2, so2, mo3, so3, mu, su, mu2, su2)    
        last_h = self.dropout(out_hn)      
        out = last_h        
        return out, h_t, c_t, l_t, lc_t, bp_t, ib, oo, ol, f, olc, h_t_tank2, c_t_tank2, l_t_tank2, lc_t_tank2, bp_t_tank2, ib_tank2, Gate_oo_tank2, ol_tank2, f_tank2, olc_tank2, h_t_tank3, c_t_tank3, l_t_tank3, lc_t_tank3, bp_t_tank3, ib_tank3, Gate_oo_tank3, ol_tank3, f_tank3, olc_tank3
...
for epoch in range(1,1+1): 
    ...
    for data in pbar:
        ...
        predictions = model(x,epoch, time_lag, y, mo, so, mo2, so2, mo3, so3, mu, su, mu2, su2)[0]      
    ...
    Result = model(x,epoch, time_lag, y, mo, so, mo2, so2, mo3, so3, mu, su, mu2, su2)
    ...
    hidden_Cascade2_eval = Result[11]
    cell_Cascade2_eval = Result[12]
    loss_Cascade2_eval = Result[13]
    lossc_Cascade2_eval = Result[14]    
    bypass_Cascade2_eval = Result[15]
    i_Cascade2_gate = Result[16]
    o_Cascade2_gate = Result[17]
    l_Cascade2_gate = Result[18]
    f_Cascade2_gate = Result[19]
    lc_Cascade2_gate = Result[20]

    hidden_Cascade3_eval = Result[21]
    cell_Cascade3_eval = Result[22]
    loss_Cascade3_eval = Result[23]
    lossc_Cascade3_eval = Result[24]    
    bypass_Cascade3_eval = Result[25]
    i_Cascade3_gate = Result[26]
    o_Cascade3_gate = Result[27]
    l_Cascade3_gate = Result[28]
    f_Cascade3_gate = Result[29]
    lc_Cascade3_gate = Result[30]
    ...
    hidden_Cascade2_eval = hidden_Cascade2_eval.detach().cpu().numpy()       
    cell_Cascade2_eval = cell_Cascade2_eval.detach().cpu().numpy()
    loss_Cascade2_eval = loss_Cascade2_eval.detach().cpu().numpy()
    lossc_Cascade2_eval = lossc_Cascade2_eval.detach().cpu().numpy()     
    bypass_Cascade2_eval = bypass_Cascade2_eval.detach().cpu().numpy()    
    i_Cascade2_gate = i_Cascade2_gate.detach().cpu().numpy()
    o_Cascade2_gate = o_Cascade2_gate.detach().cpu().numpy()
    l_Cascade2_gate = l_Cascade2_gate.detach().cpu().numpy()
    lc_Cascade2_gate = lc_Cascade2_gate.detach().cpu().numpy()
    f_Cascade2_gate = f_Cascade2_gate.detach().cpu().numpy() 

    hidden_Cascade3_eval = hidden_Cascade3_eval.detach().cpu().numpy()       
    cell_Cascade3_eval = cell_Cascade3_eval.detach().cpu().numpy()
    loss_Cascade3_eval = loss_Cascade3_eval.detach().cpu().numpy() 
    lossc_Cascade3_eval = lossc_Cascade3_eval.detach().cpu().numpy()     
    bypass_Cascade3_eval = bypass_Cascade3_eval.detach().cpu().numpy()    
    i_Cascade3_gate = i_Cascade3_gate.detach().cpu().numpy()
    o_Cascade3_gate = o_Cascade3_gate.detach().cpu().numpy()
    l_Cascade3_gate = l_Cascade3_gate.detach().cpu().numpy()
    lc_Cascade3_gate = lc_Cascade3_gate.detach().cpu().numpy()
    f_Cascade3_gate = f_Cascade3_gate.detach().cpu().numpy() 
    ...
    hidden_Cascade2_eval[hidden_Cascade2_eval < 0] = 0
    hidden_Cascade3_eval[hidden_Cascade3_eval < 0] = 0
...
# Output time series
    ...
    Out_hidden_C2[0:timeLen,epoch-1] = hidden_Cascade2_eval[:,0]       
    Out_cell_C2[0:timeLen,epoch-1] = cell_Cascade2_eval[:,0]
    Out_loss_C2[0:timeLen,epoch-1] = loss_Cascade2_eval[:,0]
    Out_lossc_C2[0:timeLen,epoch-1] = lossc_Cascade2_eval[:,0]    
    Out_o_gate_C2 [0:timeLen,epoch-1] = o_Cascade2_gate[:,0]
    Out_l_gate_C2 [0:timeLen,epoch-1] = l_Cascade2_gate[:,0]    
    Out_lc_gate_C2 [0:timeLen,epoch-1] = lc_Cascade2_gate[:,0]    
    Out_f_gate_C2 [0:timeLen,epoch-1] = f_Cascade2_gate[:,0]
    Out_hidden_C3[0:timeLen,epoch-1] = hidden_Cascade3_eval[:,0]       
    Out_cell_C3[0:timeLen,epoch-1] = cell_Cascade3_eval[:,0]
    Out_loss_C3[0:timeLen,epoch-1] = loss_Cascade3_eval[:,0]
    Out_lossc_C3[0:timeLen,epoch-1] = lossc_Cascade3_eval[:,0]    
    Out_o_gate_C3 [0:timeLen,epoch-1] = o_Cascade3_gate[:,0]
    Out_l_gate_C3 [0:timeLen,epoch-1] = l_Cascade3_gate[:,0]    
    Out_lc_gate_C3 [0:timeLen,epoch-1] = lc_Cascade3_gate[:,0]    
    Out_f_gate_C3 [0:timeLen,epoch-1] = f_Cascade3_gate[:,0]  

    print(B2)


CaseName =  "results_Cascade/Cascade-3Nodes-New/"
...
Out_file = pd.DataFrame(Out_hidden_C2)
outname = CaseName + '/' + 'Outhidden_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_cell_C2)
outname = CaseName + '/' + 'Outcell_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_loss_C2)
outname = CaseName + '/' + 'Outloss_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lossc_C2)
outname = CaseName + '/' + 'Outlossc_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_o_gate_C2)
outname = CaseName + '/' + 'Out_gateo_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_l_gate_C2)
outname = CaseName + '/' + 'Out_gatel_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lc_gate_C2)
outname = CaseName + '/' + 'Out_gatelc_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_f_gate_C2)
outname = CaseName + '/' + 'Out_gatef_Cascade2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_hidden_C3)
outname = CaseName + '/' + 'Outhidden_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_cell_C3)
outname = CaseName + '/' + 'Outcell_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_loss_C3)
outname = CaseName + '/' + 'Outloss_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lossc_C3)
outname = CaseName + '/' + 'Outlossc_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_o_gate_C3)
outname = CaseName + '/' + 'Out_gateo_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_l_gate_C3)
outname = CaseName + '/' + 'Out_gatel_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lc_gate_C3)
outname = CaseName + '/' + 'Out_gatelc_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_f_gate_C3)
outname = CaseName + '/' + 'Out_gatef_Cascade3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)