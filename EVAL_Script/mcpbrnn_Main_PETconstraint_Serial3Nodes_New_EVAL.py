...
Out_hidden_L2 = np.zeros((timeLen,1))
...
Out_cell_L3 = np.zeros((timeLen,1))
Out_loss_L3 = np.zeros((timeLen,1))
Out_lossc_L3 = np.zeros((timeLen,1))
Out_o_gate_L3 = np.zeros((timeLen,1))
Out_l_gate_L3 = np.zeros((timeLen,1))
Out_lc_gate_L3 = np.zeros((timeLen,1))
Out_f_gate_L3 = np.zeros((timeLen,1))
...
class Model(nn.Module):
    ...
    def __init__(self,
        ...
        self.MCPBRNNNode_Layer3 = MCPBRNN_Generic_PETconstraint_Scaling(input_size=input_size_dyn,
                         hidden_size=hidden_size,
                         gate_dim=gate_dim,
                         spinLen=spinLen,
                         traintimeLen=traintimeLen,              
    ...
    def forward(self, x_d, epoch, time_lag, y_eval, cmean, cstd):
        ...
        temp2 = torch.cat((h_t_L2,x_d[:,:,1]),1)
        x_d_L3 = temp2.unsqueeze(1)
        h_t_L3, c_t_L3, l_t_L3, lc_t_L3, bp_t_L3, i_L3, oo_L3, ol_L3, olc_L3, f_L3, h_nout_L3, obs_std_L3 = self.MCPBRNNNode_Layer3(x_d_L3, epoch, time_lag, y_eval, cmean, cstd)

        last_h = self.dropout(h_t_L3)      
        out = last_h      
        return out, h_t, h_t_L2, h_t_L3, c_t, c_t_L2, c_t_L3, l_t, l_t_L2, l_t_L3, lc_t, lc_t_L2, lc_t_L3, bp_t, bp_t_L2, bp_t_L3, i, i_L2, i_L3, oo, oo_L2, oo_L3, ol, ol_L2, ol_L3, olc, olc_L2, olc_L3, f, f_L2, f_L3
...
weight_file = "results_multinode/Serial-3Layers/MCPBRNN_Generic_PETconstraint_Scaling_M5_3Layer_1node_4/model_epoch2000.pt"
...
for epoch in range(1,1+1): 
    ...
    cell_eval = Result[4]
    loss_eval = Result[7]
    lossc_eval = Result[10]    
    bypass_eval = Result[13]
    i_gate = Result[16]
    o_gate = Result[19]
    l_gate = Result[22]
    lc_gate = Result[25]    
    ...
    cell_L2_eval = Result[5]
    loss_L2_eval = Result[8]
    lossc_L2_eval = Result[11]    
    bypass_L2_eval = Result[14]
    i_L2_gate = Result[17]
    o_L2_gate = Result[20]
    l_L2_gate = Result[23]
    lc_L2_gate = Result[26]    
    f_L2_gate = Result[29]

    hidden_L3_eval = Result[3]
    cell_L3_eval = Result[6]
    loss_L3_eval = Result[9]
    lossc_L3_eval = Result[12]    
    bypass_L3_eval = Result[15]
    i_L3_gate = Result[18]
    o_L3_gate = Result[21]
    l_L3_gate = Result[24]
    lc_L3_gate = Result[27]    
    f_L3_gate = Result[30]
    ...
    hidden_L3_eval = hidden_L3_eval.detach().cpu().numpy()       
    cell_L3_eval = cell_L3_eval.detach().cpu().numpy()
    loss_L3_eval = loss_L3_eval.detach().cpu().numpy() 
    lossc_L3_eval = lossc_L3_eval.detach().cpu().numpy()     
    bypass_L3_eval = bypass_L3_eval.detach().cpu().numpy()    
    i_L3_gate = i_L3_gate.detach().cpu().numpy()
    o_L3_gate = o_L3_gate.detach().cpu().numpy()
    l_L3_gate = l_L3_gate.detach().cpu().numpy()
    lc_L3_gate = lc_L3_gate.detach().cpu().numpy()    
    f_L3_gate = f_L3_gate.detach().cpu().numpy()
...
# Output time series
    ...
    Out_hidden_L2[0:timeLen,epoch-1] = hidden_L2_eval[:,0]
    ...
    Out_hidden[0:timeLen,epoch-1] = hidden_L3_eval[:,0]
    Out_cell_L3[0:timeLen,epoch-1] = cell_L3_eval[:,0] 
    Out_loss_L3[0:timeLen,epoch-1] = loss_L3_eval[:,0]
    Out_lossc_L3[0:timeLen,epoch-1] = lossc_L3_eval[:,0]
    Out_o_gate_L3[0:timeLen,epoch-1] = o_L3_gate[:,0]
    Out_l_gate_L3[0:timeLen,epoch-1] = l_L3_gate[:,0]
    Out_lc_gate_L3[0:timeLen,epoch-1] = lc_L3_gate[:,0]
    Out_f_gate_L3[0:timeLen,epoch-1] = f_L3_gate[:,0]

    print(B2)

# Save Results file
CaseName = "results_multinode/Serial-3Layers/"
...
Out_file = pd.DataFrame(Out_hidden_L2)
outname = CaseName + '/' + 'Outhidden_L2_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)
...
Out_file = pd.DataFrame(Out_cell_L3)
outname = CaseName + '/' + 'Outcell_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_loss_L3)
outname = CaseName + '/' + 'Outloss_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lossc_L3)
outname = CaseName + '/' + 'Outlossc_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_o_gate_L3)
outname = CaseName + '/' + 'Out_gateo_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_l_gate_L3)
outname = CaseName + '/' + 'Out_gatel_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_lc_gate_L3)
outname = CaseName + '/' + 'Out_gatelc_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)

Out_file = pd.DataFrame(Out_f_gate_L3)
outname = CaseName + '/' + 'Out_gatef_L3_' + str(cfg["case_no"]) + '_summary.csv'
Out_file.to_csv(outname)